...
const AdminDashboard = () => {
  ...
  const getConfidenceLevelArabic = (label) => {
    switch (label?.toLowerCase()) {
      case 'high': return <span className="text-green-400 bg-green-400/10 px-2 py-1 rounded-lg">عالي</span>;
      case 'medium': return <span className="text-orange-400 bg-orange-400/10 px-2 py-1 rounded-lg">متوسط</span>;
      case 'low': return <span className="text-red-400 bg-red-400/10 px-2 py-1 rounded-lg">منخفض</span>;
      default: return <span className="text-gray-400">{label || '-'}</span>;
    }
  };
            ...
            <table className="w-full text-right whitespace-nowrap">
              <thead className="bg-[#1E293B] text-[#94A3B8]">
                <tr>
                  ...
                  <th className="px-4 py-4 font-semibold text-sm">المسار الأول</th>
                  <th className="px-4 py-4 font-semibold text-sm">المسار التاني</th>
                  <th className="px-4 py-4 font-semibold text-sm">مستوى الثقة</th>
                  <th className="px-4 py-4 font-semibold text-sm">نسب المسارات</th>
                  <th className="px-4 py-4 font-semibold text-sm">السمات (Traits)</th>
                    ...
                    <tr key={item.id} className="hover:bg-[#1E293B]/30 transition-colors">
                      ...
                      <td className="px-4 py-4 text-sm font-bold">
                        <div className="flex items-center gap-2">
                          {getConfidenceLevelArabic(item.result_confidence?.label)}
                          <span className="text-[#94A3B8] text-xs">
                            {item.result_confidence?.score ? `(${(item.result_confidence.score * 100).toFixed(0)}%)` : ''}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-xs text-[#94A3B8]" dir="ltr">
                        {formatScores(item.result_scores)}
                      </td>
                      <td className="px-4 py-4 text-xs text-[#64748B] max-w-[200px] truncate hover:whitespace-normal hover:break-words transition-all duration-300" dir="ltr">
